...
// check if the enrollment score pop out properly for all new users
select 
member.user_id,
users.socure_enrollment_score,
tmp.sigma_v2
from analytics.looker.member_acquisition_facts member 
left join mysql_db.chime_prod.users users on users.id = member.user_id
left join (select 
to_date(api.created_at) as created_on,
api.user_id as member_id
, row_number() over(partition by user_id order by created_at) as rn          
...
and api.created_at >= '2020-07-01' 
qualify rn=1) tmp
on tmp.member_id = member.user_id
where member.enrollment_date>='2021-03-18'; -- fix day
